...
class Gravity:
    ...
    def _Draw_(self,Axis,Seta,Acc,Lagrangu,J = 0, name = "") -> list:
        ...
        if xp < 0 or xp > self.fild_size[0]:
            pass
        elif yp < 0 or yp > self.fild_size[1]:
            pass
        else:
            if self.Util == "Y" or self.Util == "y":
                ...
                for Ai in self.Set_List:
                    if Ai == name:
                        ...
                        print(f"{pos=}")
                        print(xp,yp)
                        xd = pos[0] - (Axis[0]+self.rx)
                        yd = pos[1] - (Axis[1]+self.ry)
                        ...
                        dist = (np.linalg.norm(b-a) /Ai[1]) * self.frame_t *self.frame_t

                        print(f"{dist=}")
                        
                        
                        Squid = math.sqrt((xd*xd)+(yd*yd))
                        Seta_sub = math.atan2(yd,xd)*(180/math.pi) 
...
a = Gravity(D_acc=[0,0],frame_time=1, G= 1.5, Util = "Y")
a.set_fild()
# for i in range(1):
